...
func setupTestServer(t *testing.T, mockPubSub *mockPubSub, p2pConfig *config.P2PConfig) (string, func()) {
	...
	tlsCreds, err := p2p.CreateTLSCredentials(p2pConfig, privKey[57:])
	require.NoError(t, err)
...
func TestProxyBlossomSubCreation(t *testing.T) {
	...
	_, err = p2p.NewProxyBlossomSub(p2pConfig, engineConfig, zap.NewNop(), 0)
	assert.Error(t, err)
	...
	_, err = p2p.NewProxyBlossomSub(p2pConfig, engineConfig, zap.NewNop(), 1)
	assert.Error(t, err)
	...
	proxy, err := p2p.NewProxyBlossomSub(p2pConfig, engineConfig, zap.NewNop(), 1)
	if err == nil {
...
func TestTLSConnection(t *testing.T) {
	...
	// Test with invalid TLS key (should fail to create credentials)
	invalidConfig := &config.P2PConfig{
		StreamListenMultiaddr: p2pConfig.StreamListenMultiaddr,
		PeerPrivKey:           "invalid-hex-key",
	}

	_, err = p2p.CreateTLSCredentials(invalidConfig, make([]byte, 57))
	assert.Error(t, err, "should fail to create TLS credentials with invalid key")

	// Test empty peer private key
	emptyConfig := &config.P2PConfig{
		StreamListenMultiaddr: p2pConfig.StreamListenMultiaddr,
		PeerPrivKey:           "",
	}

	_, err = p2p.CreateTLSCredentials(emptyConfig, []byte{})
	assert.Error(t, err, "should fail to create TLS credentials with empty key")
}
...
func TestTLSXSignConnection(t *testing.T) {
	...
	serverTLSCreds, err := p2p.CreateTLSCredentials(serverP2PConfig, privKeyBytes[57:])
	require.NoError(t, err, "should be able to create TLS credentials with valid Ed448 key")
	...
	servPriv, _ := hex.DecodeString(serverP2PConfig.PeerPrivKey)

	// Create proxy client that should connect with TLS
	tlsCreds, err := p2p.CreateTLSCredentials(clientP2PConfig, servPriv[57:])
	require.NoError(t, err)
...
func TestTLSKeyMismatch(t *testing.T) {
	...
	proxy, err := p2p.NewProxyBlossomSub(clientP2PConfig, clientEngineConfig, zap.NewNop(), 1)
...
func TestTLSBidirectionalVerification(t *testing.T) {
	...
	proxy, err := p2p.NewProxyBlossomSub(sharedP2PConfig, engineConfig, zap.NewNop(), 1)
...
func TestErrorHandling(t *testing.T) {
	...
	p2pConfig := &config.P2PConfig{
		StreamListenMultiaddr: "invalid-multiaddr",
		PeerPrivKey:           "invalid-key",
	}
	engineConfig := &config.EngineConfig{
		EnableMasterProxy: true,
	}
	...
	_, err := p2p.NewProxyBlossomSub(p2pConfig, engineConfig, zap.NewNop(), 1)
	assert.Error(t, err, "should fail with invalid multiaddr")

	// Test with valid multiaddr but invalid peer key
	p2pConfig.StreamListenMultiaddr = "/ip4/127.0.0.1/tcp/8080"
	p2pConfig.PeerPrivKey = "invalid-hex-key"

	proxy, err := p2p.NewProxyBlossomSub(p2pConfig, engineConfig, zap.NewNop(), 1)
	// This might succeed due to TLS fallback, but operations should fail
	if err == nil {
		defer proxy.Close()
		// Test that operations fail
		err = proxy.PublishToBitmask([]byte("test"), []byte("data"))
		assert.Error(t, err, "operations should fail when server is not available")
	} else {
		assert.Error(t, err, "should fail with invalid peer key or connection")
	}
}
...
func TestFullProxyIntegration(t *testing.T) {
	...
	var proxies []*p2p.ProxyBlossomSub
	for i := 1; i <= 3; i++ {
		proxy, err := p2p.NewProxyBlossomSub(p2pConfig, engineConfig, zap.NewNop(), uint(i))
		require.NoError(t, err)
