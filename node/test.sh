...
pushd "$NODE_DIR" > /dev/null
	CGO_LDFLAGS="-L$BINARIES_DIR -L/usr/local/lib/ -L/opt/homebrew/Cellar/openssl@3/3.6.0/lib -lbls48581 -lverenc -lbulletproofs -lvdf -lchannel -lferret -lrpm -lstdc++ -ldl -lm -lflint -lgmp -lmpfr -lcrypto -lssl" \
	CGO_ENABLED=1 \
  go test "$@"
