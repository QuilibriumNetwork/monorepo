...
impl HypergraphCrdt {
    ...
    pub fn commit(
        ...
        for shard_key in &all_shard_keys {
            let va_root = match sets.vertex_adds.get(shard_key) {
                Some(t) => t.commit(txn.as_ref(), prover)?,
                None => empty_root.clone(),
            };
            let vr_root = match sets.vertex_removes.get(shard_key) {
                Some(t) => t.commit(txn.as_ref(), prover)?,
                None => empty_root.clone(),
            };
            let ha_root = match sets.hyperedge_adds.get(shard_key) {
                Some(t) => t.commit(txn.as_ref(), prover)?,
                None => empty_root.clone(),
            };
            let hr_root = match sets.hyperedge_removes.get(shard_key) {
                Some(t) => t.commit(txn.as_ref(), prover)?,
                None => empty_root.clone(),
            };
            ...
            self.shard_metadata.write().unwrap().insert(
                shard_key.clone(),
                ShardMetadata {
                    commitment: vec![va_root.clone(), vr_root.clone(), ha_root.clone(), hr_root.clone()],
                    leaf_count,
                    size: BigInt::zero(),
                },
            );

            result.insert(shard_key.clone(), vec![va_root, vr_root, ha_root, hr_root]);
        }
